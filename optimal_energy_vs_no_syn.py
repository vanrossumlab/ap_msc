...
accs = np.linspace(92.7, 97.4, 500) #np.flip(np.geomspace(97.5, 92.7, 500))
offset = 0
unit_offsets = 0
p1s = [[],[],[],[],[],[]]
e1s = [[],[],[],[],[],[]]
...
for k in range(0, len(accs)):
    ...
    for n_units in all_n_units:
        ...
        p1s[m].append(p_connects[np.where(energies == np.min(energies))])
        e1s[m].append(energies[np.where(energies == np.min(energies))])
        ...
        plt.plot(p1s[m], e1s[m], linewidth=line_size, linestyle='None', marker='.', markersize=marker_size, color=unitRGB(n_units, True))
        #plt.plot(p1s[m], e1s[m], linewidth=1, color=unitRGB(n_units, False))
    ...
    plt.title('Optimal Synapses vs. Energy @ ' + str(np.around(acc, 1)) + "% Accuracy", fontsize=36)
...
with imageio.get_writer('opt_syn.mp4', mode='I', fps=24) as writer:
