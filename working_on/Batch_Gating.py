...
class Neural():
    ...
    def backpropagate(self, x, y, axon, dendrite):
        error = self.axon[-1] - y # cross-entropy
        self.d_weights[-1] = np.outer(error, axon[-2])
        ...
        for layer in reversed(range(0, self.n_layers-2)):
            error = self.activation_function.prime(dendrite[layer])*np.matmul(np.multiply(self.weights[layer+1], self.weight_mask[layer+1]), error)
            self.d_weights[layer] = np.outer(error, axon[layer].T)
    ...
    def train_and_test(self, n_epochs, x_train, y_train, x_test, y_test, test_interval):
        ...
        for epoch in range(0, n_epochs):
            ...
            for i in range(0, n_samples):
                sample = shuffled_idxs[i]
                den1, ax1 = self.feedforward(x_train[sample])
                den2, ax2 = self.feedforward(x_train[sample])
                if not self.inference_score(y_train[sample], ax1[-1]) or not self.inference_score(y_train[sample], ax2[-1]): 
                    dw1, db1 = self.backpropagate(x_train[sample], y_train[sample], ax1, den1)
                    self.energy += self.compute_network_energy()
                    dw2, db2 = self.backpropagate(x_train[sample], y_train[sample], ax1, den1)
                    self.energy += self.compute_network_energy()
                    for layer in range(0, len(dw1)):
                        dw1[layer] = (dw1[layer]+dw2[layer])/2
                        db1[layer] = (db1[layer]+db2[layer])/2
                    self.d_weights, self.d_biases = dw1, db1
                    self.update_weights()
                else:
                    self.d_weights = [np.zeros(w.shape) for w in self.weights]
                    self.d_biases = [np.zeros(b.shape) for b in self.biases]
