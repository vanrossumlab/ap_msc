import numpy as np
from components import Activation
...
class Neural():
    ...
    def backpropagate(self, x, y):
        ...
        self.d_weights[-1] = np.outer(error, self.axon[-2].T)/np.tile(np.count_nonzero(self.weight_mask[-1], 1), np.shape(self.weight_mask[-1])[0])
        ...
        for layer in reversed(range(0, self.n_layers-2)):
            ...
            self.d_weights[layer] = np.outer(error, self.axon[layer].T)/np.tile(np.count_nonzero(self.weight_mask[layer], 1), np.shape(self.weight_mask[layer])[0])
