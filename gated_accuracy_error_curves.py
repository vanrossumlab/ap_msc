...
for n_units in all_units:
    ...
    for i in range(0, len(data)):
        if n_units == u and int(p*100) == int(data[i]['network_parameters']['p_connect'][0]*100): #data[i]['network_parameters']['p_connect'][0] == 1.0: 
            ...
            labs.append('Ungated: ' + str(data[i]['network_parameters']['p_connect'][0]))
...
for n_units in all_units:
    ...
    for i in range(0, len(data)):
        if n_units == u and int(p*100) == int(data[i]['network_parameters']['p_connect'][0]*100): #data[i]['network_parameters']['p_connect'][0] == 1.0: 
            ...
            e = data[i]['results']['energy']
            err = data[i]['results']['error']
            s = data[i]['results']['samples_seen'][0]
            p_s.append(data[i]['network_parameters']['p_connect'][0])
            labs.append('Gated: ' + str(data[i]['network_parameters']['p_connect'][0]))
            ax.plot(s, err, color=unitRGB(u, False), linewidth=line_size)
            ax2.plot(s, a, color=unitRGB(u, True), linewidth=line_size)
            break
...
ax.set_ylim([0.05, 2])
plt.xlim([0, 1.81e6])
plt.title('Gated ' + str(u) + ' Units: Testing Accuracy & Error Curves', fontsize=36)
...
filename = 'gated_ungated_' + str(u) + '_accuracy_error_curves.svg'
fig.tight_layout()
plt.savefig(figure_path+filename, bbox_inches='tight')
filename = 'gated_ungated_' + str(u) + '_accuracy_vs_energy.png'
plt.savefig(figure_path+filename, bbox_inches='tight')
plt.show()
