...
n_units = 300
figure_path = 'data/figures/ungated/' + str(n_units) + '/'
filenames = []
...
for i in range(0, len(data)):
    ...
    min_e = data[i]['results']['min_energy']
    ...
    #h = plt.plot(full_a, full_min_e, linewidth=line_size, color=unitRGB(0, False))
    
    #plt.plot(a, e, linestyle='None', marker='.', markersize=marker_size, color=unitRGB(200, False), label='_nolegend_')
    h = plt.plot(a, e, linewidth=line_size, color=unitRGB(n_units, True))
    #h = plt.plot(a, min_e, linewidth=line_size, color=unitRGB(n_units, False))
     
    plt.legend(['Ungated: 1.0', 'Ungated: '+l], fontsize=22, loc='lower right')
    plt.title('Ungated: Energy vs. Accuracy with ' + str(n_units) + ' Units @ ' + l, fontsize=36)
    ...
    filename = 'ungated_' + str(n_units) + '_accuracy_vs_energy_' + l.replace('.', '_') + '.svg'
    plt.savefig(figure_path+filename)
    filename = 'ungated_' + str(n_units) + '_accuracy_vs_energy_' + l.replace('.', '_') + '.png'
    plt.savefig(figure_path+filename)
    plt.close()
